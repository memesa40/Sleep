...
sleep_null %>% ggplot(aes(x = stat)) +
  geom_histogram(binwidth = 0.01, fill = "#809FE3", color = "grey") +
  geom_vline(xintercept = diff_orig, color = "#D5846D", linewidth = 1.5) +
  labs(title = "Permutation Distribution of Difference in means",
       x = "Difference in means (Male - Female)",
       y = "Count")

# null hypothesis can be rejected :@ i think 
## we will calculated the p-value as well, for good measure

sleep_null %>%
  summarize(p_value = mean(abs(stat) >= abs(diff_orig)))

# finally i will create a regression model

multiple_regressions <- lm(Sleep_Quality ~ Gender + Age + Race , data = simulated_data_filtered)
